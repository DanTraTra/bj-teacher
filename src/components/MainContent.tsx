...
const MainContent: React.FC<MainContentProps> = ({
    ...
    const [GameStatsOpen, setGameStatsOpen] = useState<boolean>(false);
    ...
    // const [TutorialState, setTutorialState] = useState<number>(tutorialMode)
    ...
    // useEffect(() => { setTutorialState(tutorialMode)}, [tutorialMode])
    useEffect(() => {
    ...
    const [GlobalDeck, setGlobalDeck] = useState<CardProps[]>(TutorialState >= 0 ? initializeTutorialDeck() : initializeDeck(DeckCount));
    // const [GlobalDeck, setGlobalDeck] = useState<CardProps[]>(initializeTutorialDeck());
    ...
    const setUpGame = () => {
        ...
        const random_player_card_1 = getCardFromDeck(false, randomOn && TutorialState < 0)
        const random_dealer_card_1 = getCardFromDeck(false, randomOn && TutorialState < 0)
        const random_player_card_2 = getCardFromDeck(false, randomOn && TutorialState < 0)
        const random_dealer_card_2 = getCardFromDeck(false, randomOn && TutorialState < 0)
    ...
    useEffect(() => {
        if (TrainingMode) {
            ...
            if (CardCountingMode) {
                setDeckCount(1)
                // console.log("starting over2")

                handleClickStartOver(1)

            } else {
                setDeckCount(6)
                // console.log("starting over3")
            ...
            if (dealerHandSum < 17 && PlayerHand[PlayerHandIndex].sum <= 21 && !PlayerBlackJackState && !PlayerHand.every(hand => hand.winMultiplier == 0)) {
                ...
                if (DealerHand.every((card) => card.visible)) {
                    setTimeout(() => {
                        ...
                        // // console.log("PlayerStand || GameState == 'PLAYER BUST' || (PlayerHand.length > 1 && LOSE.includes(GameState))")
                        // // console.log(PlayerStand, GameState == 'PLAYER BUST', PlayerHand.length > 1, LOSE.includes(GameState))
        ...
        if (HANDOVER.includes(GameState) && DealerTurnEnded) {
            ...
            setTimeout(() => {
                ...
                if (PlayerHand[PlayerHandIndex].betDisplay != winAmount) {
                    ...
                    setChipAnimationOver(true)
                    // console.log("chipAnimationOver")
                }
            }, 0)
                            ...
                            <div className="flex flex-row w-full items-center space-x-2 border-gray-100 px-6">
                                <label className="w-full">
                                    <input type="text"
                                           ...
                                           required
                                           className="flex flex-row w-full h-8 text-black rounded-md px-3 font-tech items-center"
                                    />
                                </label>
                                <button className="btn btn-sm mt-auto rounded-md font-tech" type="submit">Save</button>
    ...
    const trainMenuButtons = [
        ...
        },
        {
            ...
            onClick: () => {
                setCardCountingMode(currentState => !currentState)
            },
                                    ...
                                    >
                                        <div className="flex flex-col space-y-4">
                                            <div className="flex flex-row items-end h-24 justify-center">
                                                ...
                                                <div
                                                    className="absolute flex ml-2 size-6 bottom-0.5 -right-6 rounded-lg items-center justify-center bg-info-content/60 text-white text-sm">{DealerHandSumState}</div>
                                                }
                                            </div>
                                        </div>
                                    </div>
