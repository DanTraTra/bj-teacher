...
function Blackjack() {
    ...
    useEffect(() => {
        ...
        let updatedBoard: LeaderboardRow[] = LeaderboardData.map((d, index) => {
            const hasGames = d.game_log_data && d.game_log_data.length > 0;
            return {
                rank: 1,
                player: d.username,
                cashOut: hasGames ? d.game_log_data[d.game_log_data.length - 1].EndingBalance : 0,
                hands: hasGames ? d.game_log_data.length : 0,
                win: hasGames ? parseFloat(((d.game_log_data.filter(game => game.PlayerCards.some(hand => hand.winMultiplier > 1)).length / d.game_log_data.length) * 100).toFixed(0)) : 0,
                db_index: index,
